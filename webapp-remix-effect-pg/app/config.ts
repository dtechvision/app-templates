...
export const SqlLive = Layer.unwrapEffect(
  Effect.gen(function* () {
    const pgClientLayer = yield* Config.option(Config.redacted("DATABASE_URL")).pipe(
      Effect.flatMap(
        Option.match({
          ...
          onNone: () =>
            Effect.gen(function* () {
              yield* Effect.logInfo(
                "[SqlLive] no DATABASE_URL, using individual env variables",
              );
              const database = yield* Config.nonEmptyString(
                "DATABASE_NAME",
              ).pipe(Config.withDefault("effect_pg_dev"));
              const username = yield* Config.nonEmptyString(
                "DATABASE_USER",
